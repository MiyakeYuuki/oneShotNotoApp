...
export const getKeywords = async () => {
  try {
    ...
    const keywords: string[] = keywordListSnapshot.docs.map(
      (doc) => doc.data().name
    ); // keywordの中身
    return keywords;
  } catch (error) {
...
export const getSpots = async (keywords: string[]) => {
  ...
  const keywordQueries = keywords.map((keyword) => {
    ...
    return query(
      spotsCollection,
      where("keywords", "array-contains", doc(fireStoreDB, "keywords", keyword))
    );
  });

  try {
    // クエリの実行 -> categorySnapShotsArrayに配列でまとめる
    const keywordSnapShotsArray = await Promise.all(
      keywordQueries.map((q) => getDocs(q))
    );
    ...
    keywordSnapShotsArray.forEach((keywordSnapShots) => {
      keywordSnapShots.docs.forEach((docs) => {
        ...
        const areaRef: DocumentData = doc(
          fireStoreDB,
          "area_list",
          docData.area.id
        );
        ...
        const geopoint = docData.geopoint;
        const id = docs.id;
    ...
    await Promise.all(
      Array.from(uniqueResultsMap.values()).map(async (result) => {
